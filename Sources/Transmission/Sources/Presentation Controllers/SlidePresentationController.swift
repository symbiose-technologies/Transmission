...
class SlidePresentationController: PresentationController, UIGestureRecognizerDelegate {
    ...
    private func onPanGesture(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .top:
            ...
            // Determine if the swipe is more vertical than horizontal
            let isVerticalSwipe = abs(gestureTranslation.y) > abs(gestureTranslation.x)
            
            
            print("SlidePresentationController onPanGesture: isVerticalSwipe: \(isVerticalSwipe) \(gestureTranslation) offset: \(offset) translation: \(translation) percentage: \(percentage)")
            
            // If it's not a vertical swipe, do not proceed
            if !isVerticalSwipe {
                return
            }
...
// =======
            ...
            guard otherGestureRecognizer.isScrollViewPanGesture else {
                // Cancel
                print("SlidePresentationController: Cancelling my gesture recognizer... other is: \(otherGestureRecognizer)")
//                gestureRecognizer.isEnabled = false; gestureRecognizer.isEnabled = true
                return true
            }
            
            
            let isScrollViewPanGesture = otherGestureRecognizer.isScrollViewPanGesture
